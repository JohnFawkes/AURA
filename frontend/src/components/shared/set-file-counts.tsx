...
export function SetFileCounts({ mediaItem, set, includedItems }: SetFileCountsProps) {
    ...
    const getShowFileCounts = () => {
        ...
        if (seasonPosterCount > 0)
            line2Parts.push(`${seasonPosterCount} Season Poster${seasonPosterCount > 1 ? "s" : ""}`);
        if (specialSeasonPosterCount > 0)
            line2Parts.push(
                `${specialSeasonPosterCount} Special Season Poster${specialSeasonPosterCount > 1 ? "s" : ""}`
            );
        ...
        const setTitlecards = set.images.filter(
            (img) =>
                img.type === "titlecard" &&
                typeof img.season_number === "number" &&
                typeof img.episode_number === "number"
        );
                ...
                if (epInfo.inMediaItem) {
                    ...
                    if (epInfo.setHasTitlecard) {
                        if (seasonInfo.setHasSeasonPoster) {
                            ...
                            const allEpisodesMissing = seasonInfo.episodes.every(
                                (ep) => ep.inSet && ep.setHasTitlecard
                            );
    ...
    return (
        <div>
            {mediaItem.type === "movie" && movieResult && (
                <>
                    {movieResult.primary && (
                        <Lead className="text-sm text-muted-foreground ml-1">{movieResult.primary}</Lead>
                    )}
                                    ...
                                    <div>
                                        ...
                                        {showResult.missingFromServer.length > 0 && (
                                            <>
                                                ...
                                                <ul className="text-sm list-disc list-inside">
                                                    {showResult.missingFromServer.map(
