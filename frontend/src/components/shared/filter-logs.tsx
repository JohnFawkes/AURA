...
function LogsFilterContent({
  ...
  return (
    <div className="flex-grow space-y-4 overflow-y-auto px-4 py-2">
            ...
            <div key={section}>
              ...
              {actions
                ...
                .map((action) => (
                  ...
                  >
                    <Checkbox checked={pendingActionsFilter.includes(action.value)} id={`action-${action.value}`} />
                    <Label
                    ...
                    {pendingActionsFilter.includes(action.value) && <Check className="h-4 w-4 text-primary" />}
                  </div>
                ))}
            </div>
          ))}
      ...
      <Button className="w-full mt-2 cursor-pointer hover:brightness-120 active:scale-95" onClick={handleApplyFilters}>
        Apply Filters
      </Button>
    ...
    <Dialog open={modalOpen} onOpenChange={setModalOpen}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            Use the options below to change the number of logs displayed and filter logs by level, status, or
            action/path.
          </DialogDescription>
        </DialogHeader>
