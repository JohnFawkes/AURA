...
function StarVisual({ fill }: { fill: 0 | 0.5 | 1 }) {
  return (
    <span className="relative inline-flex h-6 w-6">
      ...
      {fill === 1 && <Star className="absolute inset-0 h-6 w-6 text-yellow-500 fill-yellow-500" strokeWidth={2} />}
      {fill === 0.5 && (
        <Star
          className={cn("absolute inset-0 h-6 w-6 text-yellow-500 fill-yellow-500", "[clip-path:inset(0_50%_0_0)]")}
          strokeWidth={2}
        />
      )}
    </span>
      ...
      <DialogContent className={cn("max-h-[80vh] overflow-y-auto sm:max-w-[700px]", "border border-primary")}>
        ...
        <div className="mt-2">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center" onMouseLeave={() => setHoverRating(null)} aria-label="Rating">
